...
export default {
  ...
  setup(props, context) {
    ...
    const showInfo = (content, position) => {
      ...
      map.setCenter(mapService.infoWindow.getPosition())
    }
    /***
     * 搜索框内选择回调
     */
    ...
    const loadAMap = () => {
      AMapLoader.load({
        key: mapKey,
        version: '2.0',
        plugins: ['Map3D', 'AMap.MouseTool', 'AMap.MarkerClusterer', 'AMap.ControlBar', 'AMap.OverView', 'AMap.PlaceSearch'],
        AMapUI: {
          version: '1.1',
          plugins: []
        },
